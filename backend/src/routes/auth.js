    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                return [4 /*yield*/, database_js_1.default.query('SELECT * FROM users WHERE login=$1', [login])];
            case 1:
                rows = (_b.sent()).rows;
                user = rows[0];
                if (!user)
                    return [2 /*return*/, res.status(401).json({ error: 'Utilisateur inconnu' })];
                return [4 /*yield*/, bcryptjs_1.default.compare(password, user.password_hash)];
            case 2:
                match = _b.sent();
                if (!match)
                    return [2 /*return*/, res.status(401).json({ error: 'Mot de passe incorrect' })];
