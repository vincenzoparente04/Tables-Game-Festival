-- 02_seed_neon.sql
-- Load CSV from client (your machine) into staging, then insert into app tables.
...
-- 2) load CSVs from your local repo (client-side)
\copy stg_editeur FROM 'editeur.csv' WITH (FORMAT csv, HEADER true, DELIMITER ',', QUOTE '"');
\copy stg_typejeu FROM 'typeJeu.csv' WITH (FORMAT csv, HEADER true, DELIMITER ',', QUOTE '"');
\copy stg_jeu     FROM 'jeu.csv'     WITH (FORMAT csv, HEADER true, DELIMITER ',', QUOTE '"');
